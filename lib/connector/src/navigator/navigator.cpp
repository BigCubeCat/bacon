...
std::pair<double, double> Navigator::calculatePosition(
    const std::vector<std::pair<std::string, std::vector<BLEBeaconState>>>&
        beaconMeasurements) {
    std::vector<std::pair<BLEBeacon, double>> distances;

    std::cout << knownBeacons_.size() << std::endl;
    ...
    if (distances.size() < 3) {
        for (int i = 0; i < distances.size(); ++i) {
            std::cout << i << " " << distances[i].first.name_ << "\n";
        }
...
void Navigator::setKnownBeacons(std::vector<message_objects::BLEBeacon> newBeacons) {
